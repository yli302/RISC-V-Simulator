...
extern void stage_fetch (struct stage_reg_d *new_d_reg) {
	uint64_t instr = 0x00000000; // address of instruction
	uint64_t size_in_bytes = 4; // read 32 bits from pc
	memory_read (get_pc(), &instr, size_in_bytes); // I-cache
...
extern void stage_execute (struct stage_reg_m *new_m_reg) {
	...
	uint64_t address_rd; //it is a unsigned 64-bit value
	...
	if(current_stage_x_register->instruction!=0){
		...
		if (*(array) == 1) {  
			...
			*(int64_t *) &value_rd = signed_rd;
			...
			//register_write(register_rd, signed_rd);
		} else if (*(array) == 9) { 
			// slli
			uint64_t register_rd = *(array + 2);
			...
			*(int64_t *) &value_rd = signed_32_rs1;
			...
			address_target = value_rs1 + imm11_0;
			//register_write(register_rd, value_rd);
			...
			value_rd = current_stage_x_register -> new_pc + 0x4;
			address_target = current_stage_x_register -> new_pc +imm_20_1;
			//register_write(register_rd, value_rd);
...
extern void stage_memory (struct stage_reg_w *new_w_reg) {
	...
	if(current_stage_m_register->instruction!=0){
		uint64_t value_rd = current_stage_m_register -> value_rd;
