...
uint64_t* decode(uint64_t fetch_result) {
...
void execute(uint64_t *array, uint64_t pc, uint64_t * new_pc) {
	...
	if (*(array) == 1) {  
		...
		if (imm11_0 >= 0x800) {
			...
			imm11_0 = imm11_0 & 0x00000000000007ff;
		}
        ...
        *(int64_t *) &value_rd = signed_rd;
		...
		// append 12 low-order zero bits
		value_imm31_12 = value_imm31_12 << 12;
		// sign-extended to 64-bit
		if (value_imm31_12 >= 0x80000000) {
			value_imm31_12 = value_imm31_12 | 0xffffffff80000000;
		} else {
			value_imm31_12 = value_imm31_12 & 0x000000007ffff000;
		}
		...
		*(int64_t *) &value_rd = signed_32_rs1;
		...
		// bit mask
		value_rs2 = value_rs2 & 0x00000000000000ff;
        ...
        register_read (register_rs1, register_rs2, &value_rs1, &value_rs2);
		// bit mask
		value_rs2 = value_rs2 & 0x000000000000ffff;
        ...
        register_read (register_rs1, register_rs2, &value_rs1, &value_rs2);
		// bit mask
		value_rs2 = value_rs2 & 0x00000000ffffffff;
        ...
        *(int64_t *) &value_rd = signed_rd;
        ...
        register_read (register_rs1, register_rs2, &value_rs1, &value_rs2);
        // shift amount held in the lower 6 bits.
		value_rs2 = value_rs2 & 0x3f;
        // logical left shift
        ...
        *(int64_t *) &value_rd = signed_rd;
		...
		if (value_rd >= 0x80000000) {
			...
			value_rd = value_rd & 0x000000007fffffff;
		}
		register_write(register_rd, value_rd);
    } else if (*(array) == 37) { 
		// addw
        ...
        *(int64_t *) &value_rd = signed_32_rs1;
		...
		// extend to 64-bit
		if (imm_12_1 >= 0x1000) {
            imm_12_1 = imm_12_1 | 0xfffffffffffff000; // negative result
        } else {
            imm_12_1 = imm_12_1 & 0x0000000000000fff; // positive result
        }
		...
		*new_pc = value_rs1 + imm11_0;
		...
		imm_20 = imm20_10_1_11_19_12 & 0x80000;
		
		imm_20_1 = imm_10_1 + imm_11 + imm_19_12 + imm_20;
		imm_20_1 = imm_20_1 << 1;
		
		// extend to 64-bit
		if (imm_20_1 >= 0x100000) {
            imm_20_1 = imm_20_1 | 0xfffffffffff00000; // negative result
        } else {
            imm_20_1 = imm_20_1 & 0x00000000000fffff; // positive result
        }
